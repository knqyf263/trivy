...
var (
	...
	errUnsupportedPkgType = errors.New("unsupported package type")
)
...
func (c *BOM) UnmarshalJSON(b []byte) error {
	...
	sort.Slice(c.Applications, func(i, j int) bool {
		switch {
		case c.Applications[i].SrcType != c.Applications[j].SrcType:
			return c.Applications[i].SrcType < c.Applications[j].SrcType
		case c.Applications[i].PkgType != c.Applications[j].PkgType:
			return c.Applications[i].PkgType < c.Applications[j].PkgType
		default:
...
func (c *BOM) parseSBOM(bom *cdx.BOM) error {
	...
	for bomRef := range c.dependencies {
		...
		case cdx.ComponentTypeApplication: // It would be a lock file in a CycloneDX report generated by Trivy
			if core.LookupProperty(component.Properties, PropertySource) == "" &&
				core.LookupProperty(component.Properties, PropertyType) == "" {
				continue
			}
...
func parsePkgs(components []cdx.Component, seen map[string]struct{}) ([]ftypes.Package, error) {
	...
	for _, com := range components {
		...
		pkg, err := toPackage(com)
		if errors.Is(err, errPURLEmpty) || errors.Is(err, errUnsupportedPkgType) {
			continue
		} else if err != nil {
...
type appKey struct {
	Source  ftypes.TargetType
	PkgType ftypes.PkgType
}

func aggregatePkgs(libs []cdx.Component) ([]ftypes.PackageInfo, []ftypes.Application, error) {
	osPkgMap := make(map[ftypes.PkgType]ftypes.Packages)
	langPkgMap := make(map[appKey]ftypes.Packages)
	for _, lib := range libs {
		pkg, err := toPackage(lib)
		if errors.Is(err, errPURLEmpty) || errors.Is(err, errUnsupportedPkgType) {
			continue
		} else if err != nil {
		...
		// OS packages
		if pkg.Type.OSPkg() {
			osPkgMap[pkg.Type] = append(osPkgMap[pkg.Type], pkg.Package)
			continue
		}

		// For backward compatibility: Trivy used to store the source type
		// in the `PkgType` property on a library component level.
		source := ftypes.LangType(core.LookupProperty(lib.Properties, PropertyPkgType))
		if source == "" {
		...
		langPkgMap[key] = append(langPkgMap[key], pkg.Package)
	}
...
func toApplication(component cdx.Component) *ftypes.Application {
	srcType := ftypes.LangType(core.LookupProperty(component.Properties, PropertySource))
	if srcType == "" {
		// For SBOM generated by older Trivy versions
		srcType = ftypes.LangType(core.LookupProperty(component.Properties, PropertyType))
	}

	pkgType := ftypes.PkgType(core.LookupProperty(component.Properties, PropertyPkgType))
	if pkgType == "" {
		// For SBOM generated by older Trivy versions
	...
	if srcType == ftypes.NodePkg || srcType == ftypes.PythonPkg ||
		srcType == ftypes.GemSpec || srcType == ftypes.JAR || srcType == ftypes.CondaPkg {
		filePath = ""
	}
...
func toPackage(component cdx.Component) (*purl.Package, error) {
	...
	if !supportedPkgType(pkg) {
		log.Logger.Debugf("Skip the component (BOM-Ref: %s) as the package type (%s) is not supported",
			component.BOMRef, pkg.Type)
		return nil, errUnsupportedPkgType
	}

	pkg.Type = ftypes.PkgType(p.Type)
	pkg.Name = canonicalPkgName(pkg.Type, pkg.Name, component)
...
func supportedPkgType(pkg *purl.Package) bool {
	if pkg.Type == ftypes.PkgTypeOCI {
		return false
	}
	_, ok := ftypes.SupportedPkgTypes[pkg.Type]
	return ok
}
...
func canonicalPkgName(pkgType ftypes.PkgType, pkgNameFromPURL string, component cdx.Component) string {
	if pkgType == ftypes.PkgTypeMaven {
